...
std::string bench_nvme_cpu_vary_bw(int sf, int server_number,
  ...
  for (const auto& md_dirs : all_md_dirs) {
    ...
    auto shaper = proteus::CPUOnlyNVMeMorsel(
    ...
    auto bench_res = benchmark_query("scan_commitdate", scan_query, 5);
...
std::string bench_nvme_cpu_vary_bw_compressed(int sf, int server_number,
                                              int scan_router_slack = 2,
                                              int scan_memmove_slack = 4) {
  CHECK(sf == 100 || sf == 1000) << "sf is not 100 or 1000";
  std::stringstream result_string;
  ...
  for (const auto& md_dirs : all_md_dirs) {
    ...
    CatalogParser::getInstance().clear();
    LOG(INFO) << "running with " << md_dirs.size() << " md directories";
    auto compressed_shaper = proteus::CPUOnlyNVMeMorsel(
        md_dirs, "inputs/ssbm100", ssb::Query::getStats(sf), true,
        scan_memmove_slack, scan_router_slack, 16);
    auto scan_query = small_scan(compressed_shaper, "lo_commitdate");
    auto bench_res = benchmark_query("scan_commitdate", scan_query, 5);
    result_string << bench_res.label << ","
...
std::string bench_ssb_nvme_cpu_vary_bw(int sf, int server_number,
                                       int scan_router_slack = 2,
                                       int scan_memmove_slack = 4,
  ...
  for (const auto& md_dirs : all_md_dirs) {
    for (auto [query_prep_func, query_name] :
      ...
      CatalogParser::getInstance().clear();
      auto compressed_shaper = proteus::CPUOnlyNVMeMorsel(
          md_dirs, "inputs/ssbm" + std::to_string(sf), ssb::Query::getStats(sf),
          true, scan_memmove_slack, scan_router_slack, 16);
      auto prep_query = query_prep_func(compressed_shaper);
      auto bench_res = benchmark_query(query_name, prep_query, 5);
      result_string << bench_res.label << ","
                    << "true," << md_dirs.size() << "," << server_number << ","
                    << bench_res.average_query_time.count() << ","
                    << get_current_date_str() << "," << scan_router_slack << ","
...
int main(int argc, char* argv[]) {
  ...
  if (FLAGS_bench_varybw_cpu) {
    auto res = bench_nvme_cpu_vary_bw(FLAGS_scale_factor, FLAGS_server_number);
    ss << res;
    ss << std::endl;
  }

  if (FLAGS_bench_varybw_cpu_compressed) {
    auto res = bench_nvme_cpu_vary_bw_compressed(FLAGS_scale_factor,
                                                 FLAGS_server_number);
    ss << res;
    ss << std::endl;
  }
  ...
  if (FLAGS_bench_varybw_cpu_ssb) {
    auto res = bench_ssb_nvme_cpu_vary_bw(FLAGS_scale_factor,
                                          FLAGS_server_number, 2, 4, false);
    ss << res;
    ss << std::endl;
  }
