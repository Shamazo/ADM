...
def main():
    ...
    ORDER BY ts ASC""")
            ...
            for idx,row in enumerate(list(query_result)[1:]):
                ...
                time_to_ms(SUM(CASE WHEN _slice_with_thread_and_process_info_0.ts BETWEEN {row.ts} AND {row.ts + row.dur} THEN _slice_with_thread_and_process_info_0.dur ELSE 0 END)) AS total_duration_ms_{idx},
                 """
                duration_columns.append(f"total_duration_ms_{idx}")
