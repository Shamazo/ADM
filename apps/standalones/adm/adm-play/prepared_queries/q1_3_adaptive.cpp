...
constexpr auto query = "ssb_Q1_3";
constexpr int filter_id = 13;

static RelBuilder add_direct_path(
    SplitRelBuilder &split, size_t compute_dop,
...
PreparedStatement prepare13_adaptive_shared_ht(QueryArgs args) {
  args.morph->setQueryName(query);
  auto &topo = topology::getInstance();
  const auto compute_dop =
      args.compute_numa_nodes.size() *
  ...
  auto scan_build_date_processed =
      scan_build_date_initial
          ...
          .filter([&](const auto &arg)
                      -> expression_t {  // Q1.3 specific date filter
            return expressions::hint(
                eq(arg["d_weeknuminyear"], 6) & eq(arg["d_year"], 1994),
                expressions::Selectivity{1.0 / 364});
          })
    ...
    scan_build_date_processed =
        scan_build_date_processed
            .bloomfilter_build(
                ...
                args.pushdown_numa_nodes)  // filter_id will be 13
            .pack()
            .router(DegreeOfParallelism{compute_dop}, 4, RoutingPolicy::LOCAL,
                    DeviceType::CPU,
                    std::make_unique<SpecificCpuNumaNodeAffinitizer>(
      ...
      return path_probe_data_unpacked
          ...
          .filter([&](const auto &arg) -> expression_t {
            return expressions::hint(
                ge(arg["lo_discount"], 5) & le(arg["lo_discount"], 7) &
                    ge(arg["lo_quantity"], 26) & le(arg["lo_quantity"], 35),
                expressions::Selectivity{0.1 * 3.0 / 11});
          })
          .reduce(
              [&](const auto &arg) -> std::vector<expression_t> {
                return {(arg["lo_extendedprice"] * arg["lo_discount"])
