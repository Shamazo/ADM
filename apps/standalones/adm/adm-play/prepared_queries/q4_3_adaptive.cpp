...
constexpr int filter_id = 43;

PreparedStatement prepare43_adaptive(QueryArgs args) {
...
PreparedStatement prepare43_adaptive_shared_ht(QueryArgs args) {
  args.morph->setQueryName(query);
  ...
  auto processJoinsAndGroupBy = [&](RelBuilder probe_builder) -> RelBuilder {
    ...
    if (supp_join.has_value()) {
      CHECK(part_join.has_value() && cust_join.has_value() && date_join.has_value());
    ...
    // Case 2: Initialize joins
      ...
      part_join = supp_join.value().join(
          scan_build_part,
          [&](const auto &build_arg) -> expression_t {
            return build_arg["p_partkey"];
          },
      ...
      cust_join = part_join.value().join(
          scan_build_cust,
          [&](const auto &build_arg) -> expression_t {
            return build_arg["c_custkey"];
          },
      ...
      return date_join.value().groupby(
          [&](const auto &arg) -> std::vector<expression_t> {
            return {arg["d_year"].as("tmp", "d_year"),
                    arg["s_city"].as("tmp", "s_city"),
                    arg["p_brand1"].as("tmp", "p_brand1")};
