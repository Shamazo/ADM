...
PreparedStatement prepare_taxi_1_adaptive(QueryArgs &args,
        ...
        split
            ...
            .groupby(
                ...
                [&](const auto &arg) -> std::vector<GpuAggrMatExpr> {
                  return {GpuAggrMatExpr{expression_t{1}.as("tmp", "count"), 1,
                                         0, SUM}};
                },
                10, /*maxInputSize 10 years of 12 months*/ 256)
            .pack());
  }
    ...
    paths.emplace_back(
        split
            .path(DeviceType::CPU, DegreeOfParallelism{compute_dop},
                  std::make_unique<SpecificCpuNumaNodeAffinitizer>(
                      args.compute_numa_nodes))
            .memmove(mm_slack, DeviceType::CPU, std::vector<bool>{false, false})
            .unpack()
            .filter([&](const auto &arg) -> expression_t {
              return (le(arg["trip_distance"], trip_distance_max) &
                      gt(arg["trip_distance"], trip_distance_min));
  ...
  return first_path
      ...
      .groupby(
          ...
          10, /*maxInputSize 14 years of 12 months*/ 256)
      .sort(
          [&](const auto &arg) -> std::vector<expression_t> {
