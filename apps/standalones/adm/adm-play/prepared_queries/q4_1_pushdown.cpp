...
constexpr auto query = "ssb100_Q4_1_pushdown";

constexpr int filter_id = 4241;
...
PreparedStatement prepare41_pushdown(
  ...
  auto rel_cust = morph.scan("customer", {"c_custkey", "c_nation", "c_region"});

  auto rel_part = morph.scan("part", {"p_partkey", "p_mfgr"});

  auto scan_build_supp =
      morph.scan("supplier", {"s_suppkey", "s_region"})
  ...
  return morph
      .parallel(
          ...
          [&scan_build_supp, &move_after_pushdown, &do_pushdown, &morph](
            ...
            auto rel_date_d = build.at(0).unpack();
