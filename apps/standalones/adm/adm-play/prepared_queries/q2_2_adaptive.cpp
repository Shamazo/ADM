...
constexpr int filter_id = 22;

PreparedStatement prepare22_adaptive(QueryArgs args) {
  ...
  return first_path
      ...
      .groupby(
          [&](const auto &arg) -> std::vector<expression_t> {
            return {arg["d_year"].as("PelagoAggregate#11428", "d_year"),
                    arg["p_brand1"].as("PelagoAggregate#11428", "p_brand1")};
          },
          [&](const auto &arg) -> std::vector<GpuAggrMatExpr> {
            return {GpuAggrMatExpr{
                (arg["lo_revenue"]).as("PelagoAggregate#11428", "EXPR$0"), 1, 0,
                SUM}};
          },
          10, 64)
      .pack()
...
PreparedStatement prepare22_adaptive_shared_ht(QueryArgs args) {
  args.morph->setQueryName(query);
