...
common_args="--server_number=49 --num_iterations=3 --timestamp_file={output_dir}timestamps.csv"
...
# hardcoded to hamish
  ...
  for i in "${!bench_args[@]}"; do
    ...
    log_file="${bench_output_dir}log.txt"
      ...
      mkdir -p $bench_output_dir
      log_file="${bench_output_dir}log.txt"
      echo "perf mem record -k 1 --delay=-1 --control fd:${PERF_CTL_FD},${PERF_CTL_ACK_FD} -o perf.data -- ./selectivity-micros ${bench_args} > ${log_file} 2>&1"
      perf mem record  --delay=-1 --control fd:${PERF_CTL_FD},${PERF_CTL_ACK_FD} -o perf.data -- ./selectivity-micros ${bench_args} > ${log_file} 2>&1
